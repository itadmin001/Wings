jQuery(document).ready(function($){
	...
	$("#pricingCyclesSteps").change(function(){
		...
		//pricingCycles
		for(i=pricingCycles; i<previousPricingCycles; i++)
		{
		...
		for(var m=previousPricingCycles; m<pricingCycles; m++) {
			rowHtml += '<h3 class="pricingCycle pricingCycle' + (m+1) + '">' + css3_config.translation.Cycle + ' ' + (m+1) + '</h3>';
		...
		if($(this).val()!="-1")
		{
			...
			$.ajax({
					...
					success: function(json){
						...
						$("#tab-pricing-cycles input:not([readonly='readonly'])").val("");
	...
	function setPricingCycle(cycleID, urlHash)
	{
		...
		{
			pricingTable.children(":eq("+(i+2)+")").find(".header_row_2 .css3_grid_vertical_align").html(pricingCycle.columns.prices[i].replace(/&#39;/g, "'"));			
			if(typeof(pricingCycle.columns.button_urls[i])!="undefined" && pricingCycle.columns.button_urls[i].length)
				pricingTable.children(":eq("+(i+2)+")").find(".footer_row .css3_grid_vertical_align a").attr("href", pricingCycle.columns.button_urls[i]);
		}
	}
