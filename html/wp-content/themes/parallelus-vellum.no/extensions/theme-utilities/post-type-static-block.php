	...
	static function get_static_content($args=array()) {
		...
		if (is_null($page_data))
			...
			if (get_post_meta($id, 'content_filters', true) == 'all') {
				...
				if($is_vc_widget_sidebar) {
					$from = '/'.preg_quote('[vc_row', '/').'/';
					$to = '[vc_row_inner';
...
class Theme_StaticBlock_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct('static-block-widget', __('Static Block', 'framework'), $widget_ops, $control_ops);
	}
...
if ( ! function_exists( 'func_get_vc_row_css' ) ) :
	function func_get_vc_row_css( $content, $need_style = true ) {
		...
		if( ! empty( $matches[3][0] ) ) {
			...
			if( $pos1 !== false ) {
				...
				$style = ! empty( $css ) ? ($need_style ? 'style="'.$css.'"' : $css) : '';
			}
		}
